...
tacobell_name = []  # List of the names of all of the items in our combined database of restaurant items.
...
tacobell_item_list = []  # The list of items you should buy to maximize caloric count. Items will be added soon.
...
sorted_price_list = tacobell_price
sorted_calorie_list = tacobell_calorie
sorted_name_list = tacobell_name

# We can use the newly sorted ratio list to sort the other lists. Nifty feature in Python.
sorted_ratio_list, sorted_price_list, sorted_calorie_list, sorted_name_list = map(list, zip(*sorted(zip(tacobell_ratio, sorted_price_list, sorted_calorie_list, sorted_name_list), reverse=True)))
