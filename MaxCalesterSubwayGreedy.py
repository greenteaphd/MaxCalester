...
subway_name = []  # List of the names of all of the items in our combined database of restaurant items.
...
subway_item_list = []  # The list of items you should buy to maximize caloric count. Items will be added soon.
...
for i in range(len(subway_name)): # a ratio for every item
    ratio = subway_calorie[i] / subway_price[i]
    subway_ratio.append(ratio)
...
sorted_price_list = subway_price
sorted_calorie_list = subway_calorie
sorted_name_list = subway_name

# We can use the newly sorted ratio list to sort the other lists. Nifty feature in Python.
sorted_ratio_list, sorted_price_list, sorted_calorie_list, sorted_name_list = map(list, zip(*sorted(zip(subway_ratio, sorted_price_list, sorted_calorie_list, sorted_name_list), reverse=True)))
...
def greedy_algorithm(budget):
    ...
    sorted_price = sorted(subway_price)
