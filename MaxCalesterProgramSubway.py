...
subway_best_combo = {}
subway_best_combo_price_calorie = {}  # This dictionary will have different price point as its keys and the list of
...
with open('final_subway.csv', 'r') as f:
    ...
    for row in reader:
        ...
        subway_calorie.append(int(row[2]))
...
sorted_calorie_list = sorted(subway_calorie, reverse=True)
...
def subway_main_driver(budget):
    if subway_best_combo.get(budget):
...
def greedy_algorithm(budget):
    ...
    if calorie_price_dict.get(sorted_calorie_list[0]) <= budget <= ratio_price_dict.get(sorted_ratio_list[1]) * 2:
        subway_item_list.append("1 " + calorie_name_dict.get(sorted_calorie_list[0]) + " for the price of $"
                         + str(calorie_price_dict.get(sorted_calorie_list[0])) +
                         " with a calorie count of " + str(sorted_calorie_list[0]))

        subway_best_combo.update({budget: subway_item_list})
    ...
    for i in range(length):
        ...
        if total_price + item_price <= budget:  # If what you already spent plus the price of the item you want to purchase is less than the budget...
            ...
            total_calorie_count = total_calorie_count + name_calorie_dict.get(item_name)  # Add the desired item's caloric count to the total caloric tally
            subway_item_list.append("1 " + item_name + " for the price of $" +
                                    str(item_price) + " with a calorie count of " + str(name_calorie_dict.get(item_name)))  # Add the item to the final list

    subway_best_combo[budget] = subway_item_list  # Add the list of items that is best at the price point to dictionary so that future searches are O(1)
    subway_best_combo_price_calorie[budget] = [total_price, total_calorie_count]
