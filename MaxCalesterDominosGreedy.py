...
dominos_best_combo = {}
dominos_best_combo_price_calorie = {}
...
dominos_name = []  # List of the names of all of the items in our combined database of restaurant items.
...
dominos_item_list = []  # The list of items you should buy to maximize caloric count. Items will be added soon.
...
with open('final_dominos.csv', 'r') as f:
    ...
    for row in reader:
        ...
        dominos_calorie.append(float(row[2]))

# Below, the calorie to price ratio is created and each ratio is added to the list created earlier.
for i in range(len(dominos_name)): # a ratio for every item
    ratio = dominos_calorie[i] / dominos_price[i]
    dominos_ratio.append(ratio)
...
sorted_price_list = dominos_price
sorted_calorie_list = dominos_calorie
sorted_name_list = dominos_name

# We can use the newly sorted ratio list to sort the other lists. Nifty feature in Python.
sorted_ratio_list, sorted_price_list, sorted_calorie_list, sorted_name_list = map(list, zip(*sorted(zip(dominos_ratio, sorted_price_list, sorted_calorie_list, sorted_name_list), reverse=True)))
...
def greedy_algorithm(budget):
    ...
    sorted_price = sorted(dominos_price)
