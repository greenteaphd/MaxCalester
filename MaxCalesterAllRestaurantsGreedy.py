#   MaxCalester Program - All_Restaurants
...
all_restaurants_name = []  # List of the names of all of the items in our combined database of restaurant items.
...
all_restaurants_item_list = []  # The list of items you should buy to maximize caloric count. Items will be added soon.
...
for i in range(len(all_restaurants_name)): # a ratio for every item
    ratio = all_restaurants_calorie[i] / all_restaurants_price[i]
    all_restaurants_ratio.append(ratio)
...
sorted_price_list = all_restaurants_price
sorted_calorie_list = all_restaurants_calorie
sorted_name_list = all_restaurants_name

# We can use the newly sorted ratio list to sort the other lists. Nifty feature in Python.
sorted_ratio_list, sorted_price_list, sorted_calorie_list, sorted_name_list = map(list, zip(*sorted(zip(all_restaurants_ratio, sorted_price_list, sorted_calorie_list, sorted_name_list), reverse=True)))
